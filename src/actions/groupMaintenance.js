...
export const postGroupMaintenance = (data, history) => async dispatch => {
  ...
  const values = {
    ...
    sourcedBy: (data.sourcedBy.split(',')[0].trim()).toUpperCase(),
    creditOfficer: (data.creditOfficer.split(',')[0].trim()).toUpperCase(),
    savingsProductID: (data.savingsProductID.split(',')[0]).toUpperCase(),
    loanProductID: (data.loanProductID.split(',')[0].trim()).toUpperCase(),
