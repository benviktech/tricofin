...
const SystemSecurityUpdateUser = () => {
  ...
  const { modalCloser, openModel, modalText, modalOpener } = ModalFunction();
  ...
  const createUser = () => {
    console.log(formState);
    dispatch(createSystemUser(formState)).then(() => {
      setIsLoaded(true);
    });
  };
  ...
  const updateUser = () => {
    console.log(formState);
    dispatch(updateSystemUser(formState)).then(() => {
      setIsLoaded(true);
    });
  };
  ...
  return roles.length > 0 ? (
    ...
    <div className="page-refresh">
      <div>
        <p>
          Page Load Failed Please Click{" "}
          <button onClick={reloadPage}>
            {isLoading ? <Spinner /> : "Here"}
          </button>{" "}
          to Reload{" "}
        </p>
